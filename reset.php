...
{
$hash = $_GET['id'];
	$sql_select = sprintf("SELECT * FROM `dice_email` WHERE `hash`='%s'", mysql_real_escape_string($hash));
$result = mysql_query($sql_select);
$row = mysql_fetch_array($result);
if($row)
{
$goid = $row['user_id'];
		$sql_select = sprintf("SELECT * FROM `dice_users` WHERE `id`='%s'", mysql_real_escape_string($goid));
$result = mysql_query($sql_select);
$row = mysql_fetch_array($result);
if($row)
{
...
if($row)
{
		$sql_select = "SELECT * FROM `dice_users` WHERE `id`='".$row['user_id']."'";
$result = mysql_query($sql_select);
$row = mysql_fetch_array($result);
if($row)
{
	...
	if(time() > $row['data'])
	{
		$delite = "DELETE FROM `dice_email` WHERE user_id='$ida'";
mysql_query($delite);
$pass = $_POST['pass'];
$insert = "UPDATE `dice_users` SET `password`=$pass WHERE id='$ida'";
mysql_query($insert);
